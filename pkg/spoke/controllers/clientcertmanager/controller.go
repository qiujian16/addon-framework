...
const (
	...
	clusterNameAnnotation = "open-cluster-management.io/addon-cluster-name"
...
func NewClientCertForHubController(
	clusterName, agentName, addonName, signerName, hubKubeconfigSecretNamespace, kubeconfigSecretName string,
	hubClientConfig *restclient.Config,
	spokeCoreClient corev1client.CoreV1Interface,
	hubCSRClient csrclient.CertificateSigningRequestInterface,
...
func (c *ClientCertForHubController) sync(ctx context.Context, syncCtx factory.SyncContext) error {
	...
	if hasValidKubeconfig(secret, helpers.AgentUser(c.clusterName, c.agentName, c.addonName)) {
...
func (c *ClientCertForHubController) createCSR() (string, error) {
	subject := &pkix.Name{
		Organization: []string{helpers.AgentGroup(c.clusterName, c.addonName)},
		CommonName:   helpers.AgentUser(c.clusterName, c.agentName, c.addonName),
	}
	...
	csr := &certificates.CertificateSigningRequest{
		ObjectMeta: metav1.ObjectMeta{
			GenerateName: helpers.AgentCSRGenerateName(c.clusterName, c.addonName),
